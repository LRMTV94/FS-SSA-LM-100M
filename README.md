...
* Incorporating bounded per-channel gain parameters $\alpha_c = \sigma(\text{raw\_alpha}_c) \in (0, 1)$ accounts for a direct reduction of **$-0.0966$ in loss** (**$-4.64$ PPL**, moving from **50.50 to 45.86**).
...
```bibtex
@software{fsssa_gpt_100m,
  author  = {Lorusso, Matteo},
  title   = {FS-SSA-GPT: Causal Spiking Self-Attention on FineWeb-Edu},
  year    = {2026},
  url     = {https://github.com/LRMTV94/FS-SSA-GPT-100M}
}
```
...
# FS-SSA-GPT — Causal Spiking Self-Attention on 

[![License](https://img.shields.io/badge/License-Apache_2.0-blue.svg)](LICENSE)



This repository represents the **natural evolution** of [FS-SSA](https://github.com/LRMTV94/FS_Softmax_Free_Attention), scaling **Softmax-Free Spiking Self-Attention (FS-SSA)** from synthetic classifiers and toy benchmarks to open-domain autoregressive pretraining at the **100M+ parameter scale** on **FineWeb-Edu**.

In this architecture, continuous quadratic Softmax attention is discarded entirely in favor of causal linear recurrence powered by a few-spikes (FS) neuron operating at an ultra-low latency of **$K=2$ timesteps**. Query, key, and value vectors are quantised into signed discrete spikes, replacing dense floating-point Multiply-Accumulates (MACs) with sparse synaptic additions (ACs / SOPs).

The developmental trajectory spans three progressive scales:

1. **Part 1 — Ablation & Mechanics (TinyShakespeare):** A ~2.7M parameter character-level model (6 layers) evaluating stability, causal normalization, and threshold dynamics against a matched full-precision control;

2. **Part 2 — Synthetic Scaling Proof (TinyStories):** A ~25.1M parameter model (12 layers, GPT-2 BPE) achieving a validation loss of **1.8163 ± 0.0139** (Perplexity **6.16**), matching the loss regime of dense FP32 baselines on child-level narrative generation;

3. **Part 3 — Real-World Open-Domain Scaling (FineWeb-Edu):** A ~100M+ parameter model (16 layers, $D=768$) pre-trained on ~655M tokens of real-world educational web text, directly benchmarked head-to-head against an iso-parameter, compute-matched dense Transformer baseline.

---

### Key Findings & Open-Web Scaling Results

1. **Competitive Open-Web Convergence:** On FineWeb-Edu, the 100M+ parameter `FS-SSA K=2 ± L` model converges to a validation loss of **~3.804** (**Perplexity 44.9**). While the unquantized, compute-matched Transformer baseline equipped with exact quadratic Softmax reaches **PPL ~32–34** (loss ~3.48), the resulting delta ($\Delta \text{Loss} \approx 0.32$) represents an exceptionally narrow Pareto trade-off.
 
2. **The Neuromorphic Trade-Off:** Achieving PPL  without any Softmax operations ($\mathcal{O}(N)$ linear complexity) and operating strictly at $K=2$ latency demonstrates that discrete temporal spike accumulation does not suffer from representational collapse on open-domain web corpora. It delivers massive theoretical energy reductions by substituting dense FP16 MAC arrays with sparse integer/event accumulations.

3. **Generalization & Biological Homeostasis:** The model exhibits textbook generalization stability throughout the entire 10,000-step trajectory (Train vs. Validation gap $\le 0.04$ with zero dropout). Furthermore, activation monitoring reveals a sustained spike saturation of **~13.1%**, operating precisely within the sparse, homeostatic *edge-of-chaos* regime typical of biological neural systems.

4. **Reproducibility & Stability:** Across all seeds, the spiking architecture displays zero loss divergence, gradient explosions, or vanishing states, confirming that multi-layer spiking threshold networks can be stably trained end-to-end on massive web corpora using appropriate surrogate gradient dynamics.
...
**The prediction is supported (with the current budget).** Adding the sign makes validation loss *better* by **0.1809** (driving Perplexity down by **11.71 points**, from **70.82 to 59.11**). 

In classification, distinguishing global spatial topology rarely requires destructive cancellation. In autoregressive language modeling, however, syntactic disambiguation and negative constraints demand active inhibition: an incoming query must be capable of actively canceling competing hypotheses. Without negative spikes, all inner products are strictly additive ($Q K^T \ge 0$), turning attention into a purely cumulative blur. Signed bipolar pairs ($\pm$) decouple excitation from active suppression without reintroducing dense floating-point MACs.

---

### Beyond Sign: $\gamma$ and $\alpha$

While the bipolar representation restores the ability to suppress tokens, two additional mechanisms are strictly required to turn sparse discrete events into a scalable language model:

#### 1. Temporal Decay ($\gamma$): From Unbounded Accumulation to Content Selection

In a causal linear attention framework without Softmax, the hidden associative memory computes an unnormalized running sum $\sum_{j \le i} K_j^T V_j$. Over sequence contexts of 1024 tokens, this induces an **entropy explosion**: past tokens never fade, and early context acts as persistent background noise that drowns out local syntax.

* **Static Causal Decay ($g = 0.996$):** Enforcing a recency prior via exponential decay ($\gamma^{i-j}$) yields a massive leap of **$-0.1572$ loss** (**$-8.61$ PPL**, reaching **50.50**). It prevents unbounded state variance and restores an effective working memory horizon ($\approx \frac{1}{1-\gamma} \approx 250$ tokens), providing the baseline stability needed for extended sequences.

* **Data-Dependent Dynamic Selection ($\gamma_{\text{var}}$):** Moving from a fixed scalar to an input-dependent decay $\gamma_t = \sigma(W_\gamma x_t)$ breaks the Linear Time-Invariant (LTI) ceiling, unlocking the final **44.90 PPL**. Language is non-stationary: boundary tokens (e.g., punctuation marks or paragraph breaks) require immediate memory flushes ($\gamma \to 0$), whereas core semantic entities must be preserved across hundreds of steps ($\gamma \to 1$). Dynamic $\gamma$ transforms the static accumulator into an adaptive associative memory.
...
However, multi-seed evaluations across independent initializations demonstrate that the stochastic variance of the spiking architecture remains tight ($\sigma \ll 0.5$ PPL), confirming that the convergence dynamics are strictly reproducible and do not suffer from random initialization collapse.
...
2. **The Pareto Efficiency Frontier:** The final model achieves **PPL 37.40** within a delta of just **$+0.2645$ loss** from the dense Softmax Transformer, while discarding quadratic attention matrices ($O(N)$ linear scaling) and replacing continuous floating-point MACs with sparse discrete event accumulations at an ultra-low latency of $K=2$.
...
Three script, no packages, no subdirectories. Both run on Colab with a GPU (T4 or A100).

```bash
python -m venv .venv && source .venv/bin/activate
pip install torch matplotlib numpy

python model.py             # trains, writes fsssa_gpt_state.json (Tiny Shakespeare) 
python diagnostic.py        # trains, writes fsssa_gpt_state.json (TinyStories)
python generate.py          #
```
